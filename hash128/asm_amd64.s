...
// Byte masks and shuffle tables used by the variable-length body.
...
TEXT ·aeshash32H128(SB),NOSPLIT,$0-24
	MOVQ	p+0(FP), AX
	PXOR	X0, X0
	PINSRD	$1, (AX), X0	// lane 1, so a 32-bit value never shares a block layout with the 64-bit path
	AESENC	·aeskeysched+0(SB), X0
	AESENC	·aeskeysched+16(SB), X0
	AESENC	·aeskeysched+32(SB), X0
	MOVQ	X0, ret_Lo+8(FP)
	PEXTRQ	$1, X0, ret_Hi+16(FP)
	RET
